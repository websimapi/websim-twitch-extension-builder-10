...
// Alignment guide elements (created lazily)
let alignmentGuides = {
    v: null,
    h: null
};
...
// Helper: create or fetch alignment guide lines
function getAlignmentGuide(axis) {
    if (axis === 'v') {
        if (!alignmentGuides.v) {
...
// Helper: hide all alignment guides
function hideAlignmentGuides() {
    if (alignmentGuides.v) alignmentGuides.v.style.display = 'none';
    if (alignmentGuides.h) alignmentGuides.h.style.display = 'none';
}

// Helper: update alignment guides for a given wrapper during move/resize
function updateAlignmentGuides(wrapper) {
    ...
    if (bestV) {
        const line = getAlignmentGuide('v');
    ...
    if (bestH) {
        const line = getAlignmentGuide('h');
        line.style.display = 'block';
